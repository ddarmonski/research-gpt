...
const Chat = () => {
    ...
    const [usingLlama, setUsingLlama] = useState<boolean>(false);
    ...
    const makeApiRequest = async (question: string) => {
        ...
        if(usingLlama){
            console.log("Ich bin im LLAMA IF")
            try{
                    const response = await conversationApiLLAMA2(request, abortController.signal);
                    if(response?.body){
                        ...
                        //     role: "assistant",
                        //     content: JSON.parse(runningText)["output"]
                        // }
                        setShowLoadingMessage(false);
                        setAnswers([...answers, userMessage, ...result.choices[0].messages]);
                    }
    ...
    const parseCitationFromMessage = (message: ChatMessage) => {
        ...
        if (message.role === "tool") {
            try {
                let toolMessage;
                try{
                    toolMessage = JSON.parse(message.content) as ToolMessageContent;
                    return toolMessage.citations;
                }catch{
                    const anyMessage: any = message;
                    toolMessage = {
                        intent: anyMessage.content.intent,
                        citations: anyMessage.content.citations
                    } as ToolMessageContent
                    return toolMessage.citations
                    // console.log(toolMessage)
                    // console.log(anyMessage.content.citations)
                }
            }
            catch {
                    ...
                    <div className={styles.chatContainer}>
                        <div style={{position: "absolute", left: "2em"}}>
                            <Dropdown
                                    ...
                                    placeholder="ChatGPT 3.5"
                                    options={[{key: "gpt", text: "ChatGPT 3.5"},{key: "llama2", text: "LLAMA2",disabled: false}]}
                                    // defaultSelectedKey="gpt"
                            />
                        </div>
                        {!lastQuestionRef.current ? (
                            <Stack className={styles.chatEmptyState}>
